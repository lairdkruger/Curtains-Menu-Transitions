var meltShader = {
    ...
    fragmentShader: `
        ...
        void main() {
			float x = 0.0;
			x = smoothstep(0.0, uProgress, (uProgress * 2.0 + vNextTextureCoord.y - 1.0));
			vec4 f = mix(
				texture2D(textureActive, (vTextureActiveCoord-.5)*(1.-x)+.5), 
				texture2D(nextTexture, (vNextTextureCoord-.5)*x+.5), 
				uProgress);
			gl_FragColor = f;
        }
    `,
